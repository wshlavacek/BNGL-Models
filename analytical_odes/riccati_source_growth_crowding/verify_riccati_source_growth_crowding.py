#!/usr/bin/env python3
"""Verify Riccati source-growth-crowding dynamics."""
...
MODEL = HERE / "riccati_source_growth_crowding.bngl"
...
def analytical_solution(t: np.ndarray, s: float, r: float, q: float, x0: float) -> np.ndarray:
    """Closed-form solution for dX/dt = s + r*X - q*X^2."""
    if q <= 0:
        raise ValueError("The Riccati closed form used here requires q > 0")

    d_riccati = math.sqrt(r * r + 4 * q * s)
    x_plus = (r + d_riccati) / (2 * q)
    x_minus = (r - d_riccati) / (2 * q)
    r_init = (x0 - x_plus) / (x0 - x_minus)
    r_t = r_init * np.exp(-d_riccati * t)
    return (x_plus - r_t * x_minus) / (1 - r_t)
...
def run_bionetgen(output_dir: Path) -> Path:
    ...
    gdat = output_dir / "riccati_source_growth_crowding_ode.gdat"
    if not gdat.exists():
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="riccati_source_growth_crowding_") as tmp:
    ...
    exact_x = analytical_solution(t, s, r, q, x0)
