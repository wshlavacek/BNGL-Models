#!/usr/bin/env python3
"""Verify the Richards growth model."""
...
MODEL = HERE / "richards_growth.bngl"
...
def analytical_solution(
    ...
    """Closed-form Richards solution."""
    a_richards = (k_capacity / n0) ** b_shape - 1
    return k_capacity / (1 + a_richards * np.exp(-b_shape * r * t)) ** (1 / b_shape)
...
def run_bionetgen(output_dir: Path) -> Path:
    ...
    gdat = output_dir / "richards_growth_ode.gdat"
    if not gdat.exists():
...
def main() -> None:
    ...
    r = parameters["r"]
    k_capacity = parameters["K"]
    b_shape = parameters["b"]
    n0 = parameters["N0"]

    with tempfile.TemporaryDirectory(prefix="richards_growth_") as tmp:
    ...
    exact_n = analytical_solution(t, r, k_capacity, b_shape, n0)
    exact_fraction = exact_n / k_capacity
    exact_inflection = np.full_like(t, k_capacity / (1 + b_shape) ** (1 / b_shape))
