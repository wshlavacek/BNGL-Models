...
"${PYBNF_PY:-python3}" -u "$CAMPAIGN/run_paired_oscillating.py" --starts radius_starts_6_10.json --tag rad_b \
    --budget 300 2>> rad_stderr.log
echo "=== $(date -Iseconds) ms_segments = 8 arm, all 10 starts ==="
"${PYBNF_PY:-python3}" -u "$CAMPAIGN/run_paired_oscillating.py" --starts radius_starts.json --tag rad8 \
