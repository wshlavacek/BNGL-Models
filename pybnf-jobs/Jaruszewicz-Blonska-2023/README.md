...
(Eq 7). This is outside the PEtab-exportable subset, so both jobs use its **edition-2-native analog**
— `objective = norm_sos` (per-point relative error ≈ log-difference to first order) with
`normalization = peak` (per-series scale removal, the role of the paper's geometric-mean
normalization; the reduced model is non-dimensional, the original is dimensional). Targets carry the
paper's ρ=0.03·max floor. **Native-only.**
...
|---|---|---|---|---|
| [`reduced_combination`](reduced_combination/) | **combination experiment** (Fig 2): tonic + 5 pulsatile, WT + A20KO, **914 independent points** (S1 Table) | original model, 6 protocols × 2 genotypes | 12-model joint `de` fit, native-only | Gate 3a **reproduces the paper's reported AMD\*_WT=1.29 / AMD\*_A20KO=1.16 exactly**; Gate 3b recovers **12/13 within 3×** (only the paper's least-identifiable ε loose) · **90/100** ([VALIDATION](reduced_combination/VALIDATION.md)) |
| [`reduced_onoff`](reduced_onoff/) | **on-off** (Table 2): 2 h TNF on / 10 h off, WT + A20KO, **41 independent points** — the paper's proposed identifiability-optimal protocol | original model, on-off | `de` fit, native-only | Gate 3a AMD\*≈1.4; Gate 3b recovers **10/13 within 3×** (the 3 that drift = the paper's least-identifiable δ, ε, a_3) · **86/100** ([VALIDATION](reduced_onoff/VALIDATION.md)) |
