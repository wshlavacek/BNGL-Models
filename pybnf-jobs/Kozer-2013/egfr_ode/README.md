...
| `make_petab.py` | regenerates the committed `petab/` bundle from `egfr_ode.conf` (`export_job`); **run after any conf/model/exp change** |
| `petab/` | **generated** PEtab v2 bundle (do not hand-edit) — `problem.yaml` + `parameters.tsv` / `observables.tsv` / `measurements.tsv` + `conditions.tsv` / `experiments.tsv` + a copy of `egfr_ode.bngl` **that keeps the `generate_network`+`max_stoich` cap** |
...
The runnable `egfr_ode.conf` is the single source of truth; the exported problem is committed
...
the conf. Because the dose-response experiment perturbs the total ligand (`LT`), the bundle
includes `conditions.tsv` + `experiments.tsv`: 4 observables, 9 estimated parameters and 24
measurements over 6 conditions / 6 experiments (one per dose of the 0.001–100 nM scan; the 30 nM
time-course points attach to the default condition).

> 🔑 **The committed `petab/egfr_ode.bngl` keeps the `generate_network`+`max_stoich` cap.**
> `export_job` renders a "PEtab-clean" model by stripping the whole `begin actions` block, but
> that cap is a network *definition* (it is what makes the network finite — 923 species / 11 918
> reactions; without it, generation never terminates), and PEtab v2 has no field for it. So
> `make_petab.py` restores the directive into the copy (`_preserve_network_directives`), leaving
> it **byte-identical to `egfr_ode.bngl`** and still lint-clean — a consumer who network-generates
> the bundle gets the same finite network, not the unbounded default. (This is the one bundle
> whose model copy is not export's stripped output; the other four have no cap and copy verbatim.)
