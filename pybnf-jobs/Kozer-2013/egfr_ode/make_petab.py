#!/usr/bin/env python
"""Regenerate the committed PEtab v2 bundle for the Kozer-2013 egfr_ode job.

The runnable ``egfr_ode.conf`` is the single source of truth. Its PEtab.v2 compliance
...
any change to ``egfr_ode.conf`` / ``egfr_ode.bngl`` / ``*.exp`` so the bundle can't drift
from the conf. For Kozer-2013/egfr_ode the export emits 4 observables
...
ligand (``LT``), the export also emits the conditions and experiments tables -- 6
...
Network-cap note: this model's reaction network is finite only under a ``max_stoich`` cap,
so ``egfr_ode.bngl`` keeps a one-line ``begin actions`` block
(``generate_network({overwrite=>1,max_stoich=>{EGF=>4,EGFR=>4}})``). ``export_job`` strips
the whole ``actions`` block when rendering its "PEtab-clean" model -- correct for simulation
actions, but that cap is a network *definition*, and PEtab v2 has no field for it, so the
stripped copy would fall back to the unbounded default (the network never terminates). This
script therefore restores the ``generate_network`` directive into the copy (see
``_preserve_network_directives``): the committed ``petab/egfr_ode.bngl`` keeps the cap, is
byte-identical to the source model, network-generates to the same finite 923-species /
11 918-reaction network, and still lints clean under ``petab.v2``.
