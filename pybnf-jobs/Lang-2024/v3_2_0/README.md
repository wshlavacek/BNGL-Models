...
| `make_petab.py` | regenerates the committed `petab/` bundle from `v3_2_0.conf` (`export_job`); **run after any conf/model/exp change** |
| `petab/` | **generated** PEtab v2 bundle (do not hand-edit) — `problem.yaml` + `parameters.tsv` / `observables.tsv` / `measurements.tsv` + a copy of `v3_2_0.bngl` |
...
The runnable `v3_2_0.conf` is the single source of truth; the exported problem is committed under
...
running PyBNF. It is **generated by `make_petab.py`, never hand-written**, so it can't drift from the
conf — re-run that script after any change. The paper's own PEtab files
(`paulflang/cell_cycle_petab` v3.2.0) are PEtab **v1** (SBML-based), so this is the **first PEtab v2
rendering** of the problem: 8 observables (`observableFormula` = bare function name), 177 estimated
parameters, 4800 measurements over the single wildtype condition (no `conditions`/`experiments`
tables, since the job does not perturb).
...
- **PEtab v2 round-trip** (`scripts/petab_roundtrip.py`): export (8 observables, 177 params, 4800
  measurements) → `petab.v2` lint clean → import. **PASS.** The committed [`petab/`](petab/) bundle
  is that export, regenerated byte-for-byte by `make_petab.py` (lints clean; model copy is
  byte-identical to `v3_2_0.bngl`).
