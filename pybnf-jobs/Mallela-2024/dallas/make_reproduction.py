#!/usr/bin/env python
"""Reproduction figure for the dallas job (Mallela et al. 2024, Dallas MSA).

Simulates dallas.bngl over the paper's fit window (model day t = 0..648; day 0 = 2020-01-21,
day 648 = 2021-10-30) and overlays the model's DAILY new detected cases on the fit data
(dallas.exp) -- panel A of Fig. 2 in the paper.

Two curves are drawn:

  * **published MAP** -- the .bngl nominals, i.e. the authors' Table 1 estimate
    (Dallas/Output/adaptive_files/MLE_params.txt), which the ENTIRE 19-22-parameter
...
the job's own objective -- the negative-binomial NLL at the published dispersion r = 3.021658179294529
...
MODEL = os.path.join(HERE, "dallas.bngl")
OBS = "fDCs_Cum"                 # the fit observable (a Molecules observable: cumulative counter)
T_END = 648                      # 2021-10-30; day 0 = 2020-01-21
FREE = ["beta", "fD"]

R_DISP = 3.021658179294529          # published negative-binomial dispersion (Table 1)
MAP = dict(beta=0.304172979805100, fD=0.481101273193522)   # published MAP (== .bngl nominals)
FIT = dict(beta=0.31036618220023754, fD=0.400518959535385)   # this job's L-BFGS-B result
T0 = 14.78390174375448             # start of local transmission (pinned); the model predicts 0 cases before it
...
def main():
    ...
    exp = np.loadtxt(os.path.join(HERE, "dallas.exp"))
    et, ey = exp[:, 0], exp[:, 1]
    ...
    print("Dallas MSA -- Mallela et al. 2024, Fig. 2A  (r = %g)" % R_DISP)
    ...
    for a, logy in zip(ax, (False, True)):
        ...
        a.grid(alpha=0.25)
    fig.suptitle("Dallas MSA COVID-19 (Mallela et al. 2024, Fig. 2A)  --  "
                 "neg_bin NLL %.0f (MAP) -> %.0f (fit)"
                 % (neg_bin_nll(pred_map, ey), neg_bin_nll(pred_fit, ey)), fontsize=12)
    fig.tight_layout()
    out = os.path.join(HERE, "dallas_reproduction.png")
