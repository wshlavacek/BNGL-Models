...
> — model = Appendix Eqs. 1–40; fit target = panel A of **Fig. 3**; published MAP = **Table 1**.
...
ports the authors' own PyBioNetFit setup `~/Code/PyBNF/examples/Vax_and_Variants/Houston/`
(`Houston.bngl` / `Houston.conf` / `Houston.exp`) to the edition-2 surface.

> ✅ **The 2-parameter fit improves on the published MAP: neg_bin NLL 4776.28 → 4650.97
> (Δ = -125.32, 2.6 % lower).** This is a large shape improvement -- median relative error falls by a quarter. The published MAP is an MCMC-sampled *mode*, not an
...
**7,066,141**. Network is finite (**42 species, 88 reactions**), builds in ~0.2 s, **not heavy**.
...
Daily new detected cases over the paper's full window (one time course, `houston.exp`, t = 0..648):

| observable | design | source |
|---|---|---|
| `fDCs_Cum` (differenced → daily incidence) | time course, model day t = 0..648 (2020-01-21..2021-10-30) | `houston.exp` (authors' MSA-aggregated NYT case counts) |
...
(the ODE gives the mean), with the dispersion **fixed at the authors' published r = 2.98251**
rather than fit.
...
|---|---|---|---|---|
| `beta` | 0.286990 | 0.14–0.43 | **0.291457** | transmission rate constant (/d) |
| `fD` | 0.359294 | 0.18–0.54 | **0.309728** | fraction of new symptomatic infections detected |
...
- **Real bngsim gradient fit:** `pybnf -c houston.conf` converges to a **finite** objective
  **4650.97** (3 L-BFGS-B starts × ≤20 iterations, a few minutes).
- **Reproduction** (`houston_reproduction.png`, metric = the job's own objective): NLL
  **4776.28 → 4650.97**; median |rel err| vs. the 7-day mean **34.8 % → 25.7 %**;
  peak of the 7-day mean **+50.0 % → -3.3 %** of the data's; cumulative cases
  **+26.1 % → +8.1 %**.

## `reference/` — SBML · SED-ML · COMBINE

`reference/` ships the generated network and verified-faithful exports of this job-form model:
`houston.net`, `houston.xml` (SBML L3V2), `houston.sedml` (SED-ML L1V3 uniform time course over the
fit window, CVODE, atol/rtol 1e-7), and `houston.omex` (COMBINE archive bundling all of it plus the
...
cd pybnf-jobs/Mallela-2024/houston
pybnf -c houston.conf          # L-BFGS-B gradient fit of beta and fD
...
```python
RealWorldExample(
    folder='houston', conf='houston.conf', simulator='ode',
    observables=('fDCs_Cum',),
    system='COVID-19 transmission with vaccination and the Alpha/Delta variants, Houston MSA '
           '(Mallela 2024, DOI 10.1007/s11538-024-01258-4, Fig. 3A); ODE SEIR-type '
           ...
           'published MAP, which it improves (nll 4776.28 -> 4650.97). See VALIDATION.md.'),
# Native-only: assert export_job raises NotImplementedError (not a PEtab round-trip).
# recover={'beta': 0.286990, 'fD': 0.359294} would FAIL by design -- the fit deliberately
