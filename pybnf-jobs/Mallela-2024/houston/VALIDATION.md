# VALIDATION — Mallela-2024/houston

Primary-source validation of the PyBNF job `pybnf-jobs/Mallela-2024/houston/`.
Confidence is **earned from the gate evidence below**.

> **Confidence: 85 / 100** — every input is the authors' own (model, data, and the
> published MAP from `examples/Vax_and_Variants/Houston/`), the model file differs from the
> independently verified library model only by the removed actions block, and the gradient fit
> **improves on the published MAP**
> (neg_bin NLL **4776.28 → 4650.97**, Δ = -125.32). The deduction is that this is a
...
- Author files used: `~/Code/PyBNF/examples/Vax_and_Variants/Houston/` — `Houston.bngl`,
  `Houston.conf` (the legacy `am` / `neg_bin_dynamic` adaptive-MCMC sampler), `Houston.exp` (daily
  NYT case data, t = 0..648), and `Output/adaptive_files/MLE_params.txt` (the published MAP,
  == Table 1).

"The paper's result" for this job = **the fit of the Houston MSA daily detected-case counts**
(panel A of Fig. 3) at the published MAP.
...
| author model/job/data files | ✅ | `Vax_and_Variants/Houston/` — `.bngl` + `.conf` + `.exp` |
| authors' fit result (MAP) | ✅ | `Houston/Output/adaptive_files/MLE_params.txt` (19 params + r) |
...
|---|---|---|---|---|---|
| `houston.exp` | authors' `Houston.exp` | copied verbatim | daily new detected cases (integer counts) | byte-identical | PASS |
...
`houston.bngl` is the curated library model
`models/covid19_vaccination_and_variants_mallela2024_houston.bngl` with only the
...
| aspect | authors' model | our `houston.bngl` | verdict |
|---|---|---|---|
...
**Independent structural check:** the generated `reference/houston.net` is **identical** (modulo
comment lines) to the library model's `models/.../reference/…_houston.net`, and the
...
|---|---|---|
| **neg_bin NLL (the fit objective)** | **4776.28** | **4650.97**  (Δ -125.32, −2.6 %) |
| median \|rel err\| vs. raw daily counts | 40.1 % | 37.4 % |
| median \|rel err\| vs. the 7-day rolling mean | 34.8 % | 25.7 % |
| peak of the 7-day mean, vs. data | +50.0 % | -3.3 % |
| cumulative reported cases, vs. data | +26.1 % | +8.1 % |
| `beta` | 0.286990 | 0.291457 |
| `fD` | 0.359294 | 0.309728 |
...
**Reading the numbers.** FD drops 14 %, cutting the model's Delta-wave overshoot from +50 % to within 3 % of the data's peak 7-day mean and its cumulative excess from +26 % to +8 %.
...
3. **Real bngsim gradient fit** (`pybnf -c houston.conf`): converges to a finite objective
   **4650.97**; a few L-BFGS-B line-search trials at the box edges still return `inf` (CVODES
...
`reference/` carries `houston.net`, `houston.xml` (SBML L3V2), `houston.sedml` (SED-ML L1V3) and
`houston.omex`, produced by `bngsim.convert.net_to_omex(..., gate="full")`.

| level | check | verdict |
|---|---|---|
| L0 | syntactic validity (libsbml consistency) | pass |
| L1 | structural equivalence (species/reaction counts + topology) | pass |
| L2 | round-trip identity (max scale-relative \|Δ dy/dt\| = 6.9e-18) | pass |
...
2. **Divide-by-zero in the negative-binomial derivative.** `d(data_fit)/d(mean) =
   ...
   which is what this model predicts for the first ~12 days, before `t0` gates transmission on.
...
- The dispersion is **fixed** at the published r = 2.982510703033709 rather than fit as a nuisance, so the
