# VALIDATION — Mallela-2024/nyc

Primary-source validation of the PyBNF job `pybnf-jobs/Mallela-2024/nyc/`.
Confidence is **earned from the gate evidence below**.

> **Confidence: 85 / 100** — every input is the authors' own (model, data, and the
> published MAP from `examples/Vax_and_Variants/NYC/`), the model file differs from the
> independently verified library model only by the removed actions block, and the gradient fit
> **improves on the published MAP**
> (neg_bin NLL **5313.95 → 5235.80**, Δ = -78.15). The deduction is that this is a
> *2-parameter local refinement*, not a reproduction of the authors' full 19-parameter
...
- Author files used: `~/Code/PyBNF/examples/Vax_and_Variants/NYC/` — `NYC.bngl`,
  `NYC.conf` (the legacy `am` / `neg_bin_dynamic` adaptive-MCMC sampler), `NYC.exp` (daily
  NYT case data, t = 0..648), and `Output/adaptive_files/MLE_params.txt` (the published MAP,
  == Table 1).

"The paper's result" for this job = **the fit of the New York City MSA daily detected-case counts**
(panel A of Fig. 4) at the published MAP.
...
| author model/job/data files | ✅ | `Vax_and_Variants/NYC/` — `.bngl` + `.conf` + `.exp` |
| authors' fit result (MAP) | ✅ | `NYC/Output/adaptive_files/MLE_params.txt` (19 params + r) |
...
|---|---|---|---|---|---|
| `nyc.exp` | authors' `NYC.exp` | copied verbatim | daily new detected cases (integer counts) | byte-identical | PASS |
...
`models/covid19_vaccination_and_variants_mallela2024.bngl` with only the
...
|---|---|---|---|
| social distancing | 4 periods (n = 3); Ptau/Lambdatau step at σ, τ₁..τ<sub>3</sub> | identical | match |
...
| S₀ | 19,216,182 | identical | match |
| free parameters | 19 `X__FREE` + `r__FREE` | ids declared; conf frees only `beta`, `fD`; r fixed | scoped (deliberate) |
...
**Independent structural check:** the generated `reference/nyc.net` is **identical** (modulo
comment lines) to the library model's `models/.../reference/….net`, and the
...
|---|---|---|
| **neg_bin NLL (the fit objective)** | **5313.95** | **5235.80**  (Δ -78.15, −1.5 %) |
| median \|rel err\| vs. raw daily counts | 27.3 % | 27.9 % |
| median \|rel err\| vs. the 7-day rolling mean | 23.0 % | 22.4 % |
| peak of the 7-day mean, vs. data | +31.6 % | +43.1 % |
| cumulative reported cases, vs. data | +14.7 % | +28.8 % |
| `beta` | 0.373761 | 0.375695 |
| `fD` | 0.386776 | 0.440725 |
...
**Reading the numbers.** Beta barely moves (+0.5 %) while fD rises 14 %, buying a better fit through the long Delta tail at the cost of a slightly taller winter-2020 peak: the negative-binomial likelihood weights the many low-count days more than the few peak days.
...
an MCMC-sampled *mode* of a 19-parameter posterior rather than an optimum, an improvement
...
3. **Real bngsim gradient fit** (`pybnf -c nyc.conf`): converges to a finite objective
   **5235.80**; a few L-BFGS-B line-search trials at the box edges still return `inf` (CVODES
...
`reference/` carries `nyc.net`, `nyc.xml` (SBML L3V2), `nyc.sedml` (SED-ML L1V3) and
`nyc.omex`, produced by `bngsim.convert.net_to_omex(..., gate="full")`.

| level | check | verdict |
|---|---|---|
| L0 | syntactic validity (libsbml consistency) | pass |
| L1 | structural equivalence (species/reaction counts + topology) | pass |
| L2 | round-trip identity (max scale-relative \|Δ dy/dt\| = 3.5e-17) | pass |
...
> **Conversion gotcha.** The 353-branch empirical vaccination table `v_rate()` becomes an
...
2. **Divide-by-zero in the negative-binomial derivative.** `d(data_fit)/d(mean) =
   ...
   which is what this model predicts for the first ~10 days, before `t0` gates transmission on.
...
- Scope vs. paper: **narrower by design** — a 2-parameter gradient refinement of the published MAP,
  not a reproduction of the authors' 19-parameter adaptive-MCMC posterior. Every other
...
- The dispersion is **fixed** at the published r = 9.062524032624671 rather than fit as a nuisance, so the
