#!/usr/bin/env python
"""Reproduction figure for the nyc job (Mallela et al. 2024, New York City MSA).

Simulates nyc.bngl over the paper's fit window (model day t = 0..648; day 0 = 2020-01-21,
day 648 = 2021-10-30) and overlays the model's DAILY new detected cases on the fit data
(nyc.exp) -- panel A of Fig. 4 in the paper.

Two curves are drawn:

  * **published MAP** -- the .bngl nominals, i.e. the authors' Table 1 estimate
    (NYC/Output/adaptive_files/MLE_params.txt), which the ENTIRE 19-22-parameter
...
the job's own objective -- the negative-binomial NLL at the published dispersion r = 9.062524032624671
...
R_DISP = 9.062524032624671          # published negative-binomial dispersion (Table 1)
MAP = dict(beta=0.373760909135265, fD=0.386775933212669)   # published MAP (== .bngl nominals)
FIT = dict(beta=0.37569466688516223, fD=0.4407246180694434)   # this job's L-BFGS-B result
T0 = 9.559722923033691             # start of local transmission (pinned); the model predicts 0 cases before it
...
def main():
    ...
    print("New York City MSA -- Mallela et al. 2024, Fig. 4A  (r = %g)" % R_DISP)
    ...
    for a, logy in zip(ax, (False, True)):
        ...
        a.grid(alpha=0.25)
    fig.suptitle("New York City MSA COVID-19 (Mallela et al. 2024, Fig. 4A)  --  "
