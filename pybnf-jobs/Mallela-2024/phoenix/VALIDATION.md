# VALIDATION — Mallela-2024/phoenix

Primary-source validation of the PyBNF job `pybnf-jobs/Mallela-2024/phoenix/`.
Confidence is **earned from the gate evidence below**.

> **Confidence: 85 / 100** — every input is the authors' own (model, data, and the
> published MAP from `examples/Vax_and_Variants/Phoenix/`), the model file differs from the
> independently verified library model only by the removed actions block, and the gradient fit
> **improves on the published MAP**
> (neg_bin NLL **6324.09 → 4523.99**, Δ = -1800.10). The deduction is that this is a
...
- Author files used: `~/Code/PyBNF/examples/Vax_and_Variants/Phoenix/` — `Phoenix.bngl`,
  `Phoenix.conf` (the legacy `am` / `neg_bin_dynamic` adaptive-MCMC sampler), `Phoenix.exp` (daily
  NYT case data, t = 0..648), and `Output/adaptive_files/MLE_params.txt` (the published MAP,
  == Table 1).

"The paper's result" for this job = **the fit of the Phoenix MSA daily detected-case counts**
(panel A of Fig. 5) at the published MAP.
...
| author model/job/data files | ✅ | `Vax_and_Variants/Phoenix/` — `.bngl` + `.conf` + `.exp` |
| authors' fit result (MAP) | ✅ | `Phoenix/Output/adaptive_files/MLE_params.txt` (22 params + r) |
...
|---|---|---|---|---|---|
| `phoenix.exp` | authors' `Phoenix.exp` | copied verbatim | daily new detected cases (integer counts) | byte-identical | PASS |
...
`models/covid19_vaccination_and_variants_mallela2024_phoenix.bngl` with only the
...
| aspect | authors' model | our `phoenix.bngl` | verdict |
|---|---|---|---|
...
**Independent structural check:** the generated `reference/phoenix.net` is **identical** (modulo
comment lines) to the library model's `models/.../reference/…_phoenix.net`, and the
...
|---|---|---|
| **neg_bin NLL (the fit objective)** | **6324.09** | **4523.99**  (Δ -1800.10, −28.5 %) |
| median \|rel err\| vs. raw daily counts | 57.5 % | 34.8 % |
| median \|rel err\| vs. the 7-day rolling mean | 50.4 % | 26.3 % |
| peak of the 7-day mean, vs. data | +49.8 % | -17.2 % |
| cumulative reported cases, vs. data | +13.3 % | +8.4 % |
| `beta` | 0.285817 | 0.293103 |
| `fD` | 0.538318 | 0.459025 |
...
**Reading the numbers.** The published MAP overshoots the winter-2020 surge ~1.5x and dips an order of magnitude below the data around day 500; re-estimating beta and fD alone fixes both. This is the clearest evidence in the set that an MCMC-sampled mode is not an optimum -- and the strongest argument for the gradient job.
...
3. **Real bngsim gradient fit** (`pybnf -c phoenix.conf`): converges to a finite objective
   **4523.99**; a few L-BFGS-B line-search trials at the box edges still return `inf` (CVODES
...
`reference/` carries `phoenix.net`, `phoenix.xml` (SBML L3V2), `phoenix.sedml` (SED-ML L1V3) and
`phoenix.omex`, produced by `bngsim.convert.net_to_omex(..., gate="full")`.
...
> **Conversion gotcha.** The 341-branch empirical vaccination table `v_rate()` becomes an
...
2. **Divide-by-zero in the negative-binomial derivative.** `d(data_fit)/d(mean) =
   ...
   which is what this model predicts for the first ~14 days, before `t0` gates transmission on.
...
- The dispersion is **fixed** at the published r = 3.116234553368695 rather than fit as a nuisance, so the
