...
still best on a cluster (the published fit used a large population there). The exported PEtab v2
problem is committed as a generated byproduct under [`tlbr/petab/`](tlbr/petab/) (regenerate with
`make_petab.py`) — a `conditions`/`experiments`-bearing bundle, since the dose-response scan
perturbs the total ligand.
