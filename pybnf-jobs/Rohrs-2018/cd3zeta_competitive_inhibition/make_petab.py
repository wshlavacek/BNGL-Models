#!/usr/bin/env python
"""Regenerate the committed PEtab v2 bundle for the Rohrs-2018 competitive-inhibition job.

The runnable ``cd3zeta_competitive_inhibition.conf`` is the single source of truth. Its
PEtab.v2 compliance is a *verified round-trip property* (see
``skills/curate-pybnf-job/references/petab-compliance.md``): ``pybnf.petab.export_job``
transcribes the native edition-2 job into a PEtab v2 problem that lints clean under
``petab.v2`` and re-imports. This script commits that exported problem as an illustrative
``petab/`` byproduct -- the same pattern PyBNF's own
``examples/tutorial/12_petab_roundtrip/petab/`` uses -- so consumers can pick up PEtab v2
directly without running PyBNF.

Do NOT hand-edit anything under ``petab/``: it is generated. Re-run this script after any
change to the ``.conf`` / ``.bngl`` / ``.exp`` files so the bundle cannot drift from the
conf. For this job the export emits 6 observables (``observableFormula`` = bare function
name, one per CD3zeta ITAM tyrosine), 7 estimated parameters and 312 measurements over 9
experiments and 6 conditions -- one condition per tyrosine-to-phenylalanine point mutant,
each switching that site's ``live_<site>`` gate to zero. The noise is a constant unit
sigma, so the objective is plain sum-of-squares, which is the sum of squared error the
paper itself scores with.

Provenance note: Rohrs et al. (2018) distribute their models as BioNetGen files
(Data S1-S5) and report the fit in figures; there is no author-supplied PEtab problem, so
this bundle is the first PEtab rendering of their parameter-estimation problem.

The output is normalized to the repo's file conventions (LF endings, no trailing
whitespace, single final newline) so it is byte-stable under pre-commit and a fresh run
reproduces the committed bytes exactly. Normalization only drops the empty delimiter of
trailing all-empty columns (``noisePlaceholders`` / ``noiseParameters``); the bundle still
lints clean and re-imports (verified).

Requires the PyBNF environment's interpreter (imports ``pybnf``; the lint self-check also
needs the optional ``pybnf[petab]`` extra) and BNGPATH set:
...
def _counts(out_dir: Path) -> str:
    ...
    if (out_dir / "experiments.tsv").is_file():
        parts.append(f"{len(_rows('experiments.tsv'))} experiment periods")
    return ", ".join(parts)
