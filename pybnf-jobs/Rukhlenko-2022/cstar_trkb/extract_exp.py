#!/usr/bin/env python
"""Extract the TrkB BDNF-stimulation fold-change fit target (Fig. 4A of PMC9644236).
...
For each model observable we take the SH-SY5Y/TrkB + BDNF, DMSO (no inhibitor) columns
...
def series(header, rows, antibody, cell="TrkB"):
...
def main():
    ...
    exp = "\n".join(lines) + "\n"
    with open("cstar_trkb.exp", "w") as fh:
        fh.write(exp)
