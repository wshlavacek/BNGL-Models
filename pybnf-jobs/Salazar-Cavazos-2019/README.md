...
(chi_sq 361 vs. 360), confirming the reported parameters are the global optimum. The exported
PEtab v2 problem is committed as a generated byproduct under
[`egfr_simpull/petab/`](egfr_simpull/petab/) (regenerate with `make_petab.py`) — a
`conditions`/`experiments`-bearing bundle, since the dose-response scan perturbs `EGFconc`.
