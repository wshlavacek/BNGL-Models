#!/usr/bin/env python
"""Regenerate the committed PEtab v2 bundle for the Suofu-2017 mito_camp job.

The runnable ``mito_camp.conf`` is the single source of truth. Its PEtab.v2 compliance
is a *verified round-trip property* (see ``skills/curate-pybnf-job/references/
petab-compliance.md``): ``pybnf.petab.export_job`` transcribes the native edition-2 job
into a PEtab v2 problem that lints clean under ``petab.v2`` and re-imports. This script
commits that exported problem as an illustrative ``petab/`` byproduct -- the same pattern
PyBNF's own ``examples/tutorial/12_petab_roundtrip/petab/`` uses -- so consumers can pick
up PEtab v2 directly without running PyBNF.

Do NOT hand-edit anything under ``petab/``: it is generated. Re-run this script after any
change to ``mito_camp.conf`` / ``mito_camp.bngl`` / ``*.exp`` so the bundle cannot drift
from the conf. For Suofu-2017/mito_camp the export emits 2 observables
(``observableFormula`` = the bare function names ``camp_CY_pct`` and ``camp_ML_pct``),
14 estimated parameters and 77 measurements, plus the conditions and experiments tables:
6 condition rows (two parameter targets each for ``basal``, ``melatonin`` and ``damgo``)
over two experiments, each pre-equilibrated under ``basal``. Neither ``.exp`` carries ``_SD`` columns -- Fig. 4H/4I
report SEM but the bars overlap too densely in the plateau to digitize per point -- so the
objective is plain ``sos``.

This job's model has no ``begin actions`` block at all (its network is finite without a
cap), so ``_preserve_network_directives`` below is a no-op here; it is kept so the script
stays interchangeable with the other jobs in this repository.
