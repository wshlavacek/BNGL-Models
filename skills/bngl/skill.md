...
## 0.1 Scope: this document governs content, not process

**The new-model workflow lives in `skills/curate-model/SKILL.md`.** That skill owns
which artifacts a model folder contains, the two verification levels, the shape of the
verification artifact, and the completion criteria. Follow it when curating a model, and
treat this document as the contract for what goes *inside* the artifacts it asks for.

The two used to define the workflow separately and drifted apart. If they ever disagree
again, **`curate-model` wins on process and this document wins on content.**

Which section here governs which artifact:

| artifact | governed by |
|---|---|
| folder and file names | §4.8 |
| `.bngl` structure, block order, formatting | §2, §3 |
| naming of molecules, sites, parameters, observables | §4 |
| units convention and population conversion | §1.3 |
| header and entity annotation (`#@title`, `#@figure`, …) | §6 |
| `metadata.yaml` schema, source tags, file roles | §5 |
| observables every model must expose | §7.2 |
| overlay plot style for the verification figure | §2 |
| lint rules a finished file must pass | §9 |
...
**Verification (REQUIRED when `#@figure:` is present):** A `#@figure:` tag is a claim that the protocol reproduces a published figure, and every such claim must be independently verified. **What that verification must contain, and what form it takes, are defined by `skills/curate-model/SKILL.md`** — it requires two levels (agreement with an independent implementation *and* with the reported data), and it permits either a notebook or a driver script depending on the campaign. Do not treat a notebook as mandatory here; that rule moved, and this section is not its owner. What remains house style is the *figure* the verification emits:
...
The verification artifact is the proof that the `#@figure:` claim is valid, and the figure it saves is always `verify_<author><year>.png` — a notebook shares that stem (`verify_gardner2000.ipynb` → `verify_gardner2000.png`), while a driver script keeps its own (`run_dolan2015.py` → `verify_dolan2015.png`). Naming, the notebook-versus-driver-script choice, and the rule for a folder that needs more than one verification file are all specified in `skills/curate-model/SKILL.md`.
...
4. **Reference simulation data** lives in a `reference/` subdirectory within each model folder. This directory contains committed output files (`.net`, `.gdat`, `.cdat`, `.scan`, `.xml`, `.species`, and scan subdirectories) that serve as regression baselines. Reference files SHOULD be listed in the `files` manifest with `role: reference` and paths relative to the model folder (e.g., `reference/model_ode.gdat`).
   ...
   **A network-free `.bngl` commits its `.xml` (REQUIRED).** For a model simulated network-free
   the XML is what every engine actually reads, so it is the structural baseline that the `.net`
   is for a generate-first model. Each `.bngl` whose uncommented actions block is network-free
   (`method=>"nf"`, or `writeXML()` alone) commits one `reference/<stem>.xml` — or
   `reference/<stem>_<suffix>.xml` under the protocol's own suffix, which is what the folders
   predating this rule use — written at that file's **shipped nominals**, not at the dose or
   subvolume a cross-check runs at. The rule and its rationale are stated once, in
   `skills/curate-model/SKILL.md`, "Reproducibility Of Committed Reference Output"; it is
   enforced as an ERROR (§9.4a).
...
- **ERROR**: every `.bngl` whose uncommented actions block is network-free — a `simulate()`/`parameter_scan()` with `method=>"nf"`, or a `writeXML()`-only protocol — has a committed `reference/<stem>.xml` (§5.4 rule 4). The check is structural: it does not run BioNetGen and so cannot tell whether the XML is current, only whether it exists.
...
| 1.2 | 2026-08-06 | **A network-free `.bngl` commits its `.xml`** (§5.4 rule 4, enforced as ERROR in §9.4a). The XML is what NFsim and RuleMonkey actually read, so it is the network-free counterpart of the `.net` a generate-first model commits, and the three-arm cross-check of `curate-model/references/network-free-verification.md` §3 is only meaningful if the engines were handed identical initial conditions. The parameterization convention — **shipped nominals**, not the reduced dose or subvolume a cross-check runs at — is stated once in `skills/curate-model/SKILL.md`, "Reproducibility Of Committed Reference Output". | Seven folders regenerated and committed nine XMLs: `lambda_switch_arkin1998` (both `_fullcircuit*`, whose entire actions block is `writeXML()`), `erbb_receptor_signaling_creamer2012`, `p53_nhej_dolan2015`, `tcr_signaling_chylek2014`, `tlbr_solution_macken1982`, `blbr_heterogeneity_goldstein1980` and `iron_homeostasis_host_pathogen_ghosh2011` (both files). Nothing grandfathered — the rule is green over the whole collection. |
